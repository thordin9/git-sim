...
def temp_git_repo():
    ...
    for i in range(3):
        ...
        subprocess.run(["git", "add", "test.txt"], cwd=temp_dir, check=True, capture_output=True)
        subprocess.run(["git", "commit", "-m", f"Commit {i+1}"], cwd=temp_dir, check=True, capture_output=True)
...
class TestIntegration:
    ...
    async def test_execute_log_command(self, temp_git_repo):
        ...
        if result.get("media_path"):
            ...
            assert result["media_path"].endswith(".jpg") or result["media_path"].endswith(".png")
