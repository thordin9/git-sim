        ...
        if success and stdout_str:
            ...
            if options.get('output_only_path', False):
                ...
                for line in stdout_str.split('\n'):
                    if 'git-sim_media' in line or line.endswith(('.jpg', '.png', '.mp4', '.webm')):
                        # Extract path from output
                        parts = line.strip().split()
                        for part in parts:
                            if Path(part).exists() and part.endswith(('.jpg', '.png', '.mp4', '.webm')):
...
async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> Sequence[TextContent | ImageContent | EmbeddedResource]:
    ...
    try:
        ...
        options = {
            ...
            "output_only_path": arguments.get("output_only_path", True),  # Enable by default for easier parsing
        ...
        return [
            TextContent(
                type="text",
                text=f"Error executing git-sim: {str(e)}"
            )
        ]
